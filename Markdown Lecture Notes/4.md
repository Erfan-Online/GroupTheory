...
i.e.
$$
f(g) = \text{ some isomorphism }: G \rightarrow G
$$
...
This $f(g)$ is an isomorphism of _Sets_, it is bijective. But the problem is that is isn't an isomorphism of _Groups_!
$$
f(g)(hh') \stackrel{must}{=} f(g)(h) \times f(g)(h') \\
f(g)(hh') = ghh' \\
f(g)(h) \times f(g)(h') = ghgh' \\
\Rightarrow f(g)(hh') \neq f(g)(h) \times f(g)(h')
$$
...
Before any confusion occurs, note that both $f$ and $f(g)$ are maps and we need to research the properties of both of them.

Note that $f$ is itself a homomorphism:
$$
f(gg') = f(g) \circ f(g')
$$
Remember that the group operation between automorphisms ($f(g)$s) is composition.
